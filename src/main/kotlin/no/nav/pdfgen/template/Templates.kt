...
fun setupHandlebars(env: Environment) = Handlebars(FileTemplateLoader(templateRoot.toFile())).apply {
    registerNavHelpers(this, env)
    infiniteLoops(true)
}

fun loadTemplates(env: Environment): TemplateMap = Files.list(templateRoot)
    .filter {
    ...
    .flatMap { (applicationName, templateFiles) ->
        templateFiles.map {
            ...
            val xhtml = setupHandlebars(env).compile(StringTemplateSource(fileName, templateBytes))
